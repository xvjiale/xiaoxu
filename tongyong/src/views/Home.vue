    ...
    <el-col :span="8" style="padding-right: 10px">
      <el-card class="box-card">
        <div class="user">
          ...
          <div class="userinfo">
            <p class="name">Abmin</p>
            <p class="access">超级管理员</p>
          </div>
        </div>
        <div class="login-info">
          <p>上次登录时间:<span>2021-10-25</span></p>
...
export default {
  ...
  mounted() {
    getData().then(({ data }) => {
      ...
      const { ordeData ,userData ,videoData} = data.data;

      // 线形图
      const xAxis = Object.keys(ordeData.data[0]);
      const xAxisData={
        data:xAxis
      }
      ...
      xAxis.forEach(key=>{
        echarts1Option.series.push({
          name:key,
          data:ordeData.data.map(item=>item[key]),
          type:'line'
        })
      })
      ...
      // 柱状图
       ...
       const  echarts2Option={
        ...
        xAxis:{
          type:'category',
          data:userData.map(item=>item.data),//柱状图数据
          axisLine:{
            lineStyle:{
              color:'#17b3a3'
            }
